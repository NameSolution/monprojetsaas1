...
npm start
```
Before running the server copy `.env.example` to `.env` and adjust the values.  At minimum `JWT_SECRET` and `STRIPE_SECRET` must be set so authentication and billing work correctly. `AI_API_URL` controls the endpoint used for AI responses (defaults to `https://openrouter.ai/api/v1` in the sample file) and `AI_API_KEY` provides the OpenRouter token. Update `DB_USER` and `DB_PASSWORD` if your local PostgreSQL credentials differ from the defaults.  The seed script now also creates a `hotel_customizations` table so that settings from the Personnalisation menu persist between sessions.
